...
const systems = [
  { id: 'openshell', label: 'OpenShell', sub: 'OCSF Events', color: 'var(--green)', x: 60, y: 60 },
  { id: 'kagenti', label: 'Kagenti', sub: 'OTEL Spans', color: 'var(--cyan)', x: 60, y: 180 },
  { id: 'governance', label: 'Governance', sub: 'Authority Decisions', color: 'var(--purple)', x: 60, y: 300 },
]

const ledger = { x: 420, y: 150, w: 200, h: 80 }

export function SystemDiagram() {
  return (
    <div className="card" style={{ padding: 32 }}>
      <h3 style={{ marginBottom: 24, fontSize: 18 }}>Architecture</h3>
      <svg width="100%" viewBox="0 0 900 380" style={{ maxWidth: 900 }}>
        {/* Source systems */}
        {systems.map((s, i) => (
        ...
        {systems.map((s, i) => (
          <motion.g key={`arrow-${s.id}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 + i * 0.15 }}>
            ...
            />
            <text x={310} y={s.y + 25} textAnchor="middle" fill="var(--text-dim)" fontSize={9} fontFamily="var(--font-mono)">adapter</text>
          </motion.g>
        ))}
        ...
        {/* Output: proof-explorer */}
        <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.6 }}>
          <line x1={ledger.x + ledger.w} y1={ledger.y + ledger.h / 2} x2={700} y2={ledger.y + ledger.h / 2}
            stroke="var(--text-disabled)" strokeWidth={1} strokeDasharray="3 3" />
          <rect x={700} y={ledger.y + ledger.h / 2 - 30} width={160} height={60} rx={8}
            fill="var(--surface-2)" stroke="var(--border)" strokeWidth={1} />
          <text x={780} y={ledger.y + ledger.h / 2 - 5} textAnchor="middle" fill="var(--text-secondary)" fontSize={12} fontFamily="var(--font-display)" fontWeight={600}>
            Proof Explorer
          </text>
          <text x={780} y={ledger.y + ledger.h / 2 + 12} textAnchor="middle" fill="var(--text-dim)" fontSize={10} fontFamily="var(--font-mono)">
            verify · query · drift
          </text>
        </motion.g>
        ...
        {systems.map((s, i) => (
          <motion.circle
            key={`dot-${s.id}`}
            r={4} fill={s.color}
            initial={{ cx: 220, cy: s.y + 30, opacity: 0 }}
            animate={{
              cx: [220, ledger.x],
